...
<body>
    ...
    <main>
        ...
        <form action="">
            <a href="{{ route('seeAddUserForm') }}">Ajouter un utilisateur</a>
            @if ((isset($users) AND !empty($users)))
                <input type="text" id="searchInput" placeholder="Rechercher..." onkeydown="return (event.keyCode!=13);" data-element="user">
            @endif
        </form>
        <div id="boardAndBlocPagination">
            @include('layout.adminUserBoardAndBlocPagination')
        </div>
    </main>
